...
class AuthRepository @Inject constructor(private val api: ApiService) {
    ...
    suspend fun createNewUser(userInfo: CreateUserRequestBody): Response<CreateUserResponse> {
        return api.createNewUser(userInfo)
    }

    suspend fun createAuthToken(tokenRequestBody: TokenRequestBody): Response<String> {
